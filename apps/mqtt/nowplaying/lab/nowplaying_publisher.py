...
# A small, clean 5x7 pixel font (uppercase + digits + punctuation) — proper letterforms
# with 1px inter-char spacing so strokes never touch/merge (unlike PIL's cramped default).
FONT = {
    " ": ["     ", "     ", "     ", "     ", "     ", "     ", "     "],
    "A": [".###.", "#...#", "#...#", "#####", "#...#", "#...#", "#...#"],
    "B": ["####.", "#...#", "#...#", "####.", "#...#", "#...#", "####."],
...
FONT_Y = 5              # top row of the 7-tall glyphs on the 16-tall canvas


def _text_mask(text):
    """Render text with the embedded 5x7 pixel font, 1px inter-char gap. Returns (mask, width)."""
    glyphs = [FONT.get(c, FONT[" "]) for c in text]
    gap = 1
    tw = max(1, sum(len(g[0]) for g in glyphs) + gap * max(0, len(glyphs) - 1))
